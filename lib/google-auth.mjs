...
export const TOKEN_PATH = path.join(projectDir, ".google-token.json");
const LEGACY_TOKEN_PATH = path.join(projectDir, ".gsc-token.json");
...
function loadOAuthClientSecrets() {
  const orderedPaths = [];
  const fromEnv = envFirst("GOOGLE_OAUTH_CLIENT_JSON", "GSC_OAUTH_CLIENT_JSON");
  if (fromEnv) orderedPaths.push(fromEnv);
  orderedPaths.push(path.join(projectDir, "google-oauth-client.json"));
...
function readTokenPath() {
  if (fs.existsSync(TOKEN_PATH)) return TOKEN_PATH;
  if (fs.existsSync(LEGACY_TOKEN_PATH)) return LEGACY_TOKEN_PATH;
  return TOKEN_PATH;
}
...
async function obtainOAuthTokens(oauth2Client, port) {
  ...
  if (fs.existsSync(LEGACY_TOKEN_PATH) && LEGACY_TOKEN_PATH !== TOKEN_PATH) {
    fs.unlinkSync(LEGACY_TOKEN_PATH);
  }
  return oauth2Client;
}
...
OAuth (sign in with browser - recommended for local use):
  ...
  - Download the JSON and either:
      ...
      - set GOOGLE_OAUTH_CLIENT_JSON=/absolute/path/to/client_secret....json
...
export async function authenticateGoogle({ force = false } = {}) {
  ...
  const port = Number(envFirst("GOOGLE_OAUTH_PORT", "GSC_OAUTH_PORT")) || DEFAULT_OAUTH_PORT;
